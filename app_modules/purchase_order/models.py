...
class PurchaseOrder(BaseModel):
    ...
    warehouse = models.ForeignKey(Warehouse, verbose_name=_("Warehouse"), on_delete=models.CASCADE, null=True, blank=True)
    ...
    def get_product_item_count(self):
       return self.purchase_order_products.count()
    
    @property
    def get_paid_amount(self):
       from app_modules.vendors.models import VendorBill
       vendor_bill = VendorBill.objects.filter(purchase_order__id=self.id).first()
       return vendor_bill.paid_amount if vendor_bill else 0
    
    @property
    def get_total_amount(self):
        from app_modules.vendors.models import VendorBill
        vendor_bill = VendorBill.objects.filter(purchase_order__id=self.id).first()
        return vendor_bill.bill_amount if vendor_bill else 0
    
    @property
    def get_due_amount(self):
        from app_modules.vendors.models import VendorBill
        vendor_bill = VendorBill.objects.filter(purchase_order__id=self.id).first()
        return vendor_bill.due_amount if vendor_bill else 0
...
class PurchaseOrderProducts(BaseModel):
    ...
    product = models.ForeignKey(Product,on_delete=models.CASCADE,null=True, related_name="products_purchase_order")
    ...
    def __str__(self):
        return self.purchase_order.vendor.first_name
