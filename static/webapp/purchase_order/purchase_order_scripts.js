$('#id_company, #id_vendor, #id_product, #id_unit_type, #id_status, #id_warehouse_id').select2({
    width: "100%",
});
...
$(document).ready(function() {
    ...
    $("#id_product").on('change', function(event, piece) {
        //console.log(" piece: ", piece)
        $this = $(this);
        if ($this.val() != '') {
            ajax_call_get_product_details($this, piece)
    ...
    $("#id_barcode").on('keyup',function(){
        if ($(this).val() != '') {
            ajax_call_get_product_by_barcode($(this));
...
function ajax_call_get_product_details($this, piece = '') {
    ajax_call_setup();
    $.ajax({
...
function ajax_call_get_product_by_barcode($this){
...
4
            ...
            // console.log(data.barcode_product_unit_type)
            // console.log(data.piece)
            
            $("#id_product").val(data.product_id).change([data.piece]);
            $("#id_product option[value=" + data.product_id + "]").attr('selected', 'selected');
            $("#id_product").select2().trigger('change', [ data.piece ]);

            // console.log("data.piece: ", data.piece)
            // $("#id_unit_type").trigger('change', [ data.piece ])

            // $("#id_unit_type").val(data.piece).trigger('change')
            // $("#id_unit_type").val(data.piece).change();
            // $("#id_unit_type option[data-product-type=" + data.barcode_product_unit_type + "]").attr('selected', 'selected');
            // $("#id_unit_type").select2().trigger('change');

            // $("#id_unit_type").val(data.barcode_product_unit_type).change();
            // $("#id_unit_type option[data-product-type=" + data.barcode_product_unit_type + "]").attr('selected', 'selected');
            // $("#id_unit_type").select2().trigger('change');
            // if ($('#table_added_products').has('.no-product-row').length > 0) {
            //     $('.no-product-row').remove();
...
$(document).on("change", "#id_status", function() {
    if ($(this).val() == "completed") {
        ...
        $.ajax({
            ...
            success: function(data) {
                $("#id_warehouse_id").empty();
                $("#id_warehouse_id").append(data.warehouses);
                //$("#warehouse-div").style.display = 'block';
                $("#warehouse-div").show();
            }
        ...
        $("#warehouse-div").hide();
        $("#form_purchase_order #id_warehouse_id").empty();
    }
})
